...
# user variables
my $VERS = "0.0.7 2015-11-30";  # move to scripts repo
...
sub process_input($) {
    ...
    $cmake .= "# CMakeScripts or use the ones that come by default with CMake.\n";
    $cmake .= "# set(CMAKE_MODULE_PATH \${CMAKE_SOURCE_DIR}/CMakeModules)\n\n";
    ...
    $cmake .= " \${${pname}_HDRS_MOC} \${${pname}_HDRS}" if ($add_hdrs);
    $cmake .= " \${${pname}_FORMS_HDRS}" if ($add_forms);
    $cmake .= " \${${pname}_RESOURCES_RCC}" if ($add_rsrcs);
    $cmake .= " \${${pname}_OTHERS}" if ($add_others);
    $cmake .= " )\n";

    ${$rcm} .= "target_link_libraries( $pname \${QT_LIBRARIES} \${extra_LIBS} \${add_LIBS} )\n";
    ...
    # end of file
    $cmake .= "\n";
    $cmake .= "# eof - original generated by $pgmname, v.$VERS, on ".lu_get_YYYYMMDD_hhmmss(time())."\n";

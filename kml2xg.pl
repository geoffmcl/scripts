#!/usr/bin/perl -w
# NAME: kml2xg.pl
# AIM: Given a Google Earth xml file, render as an xg 2D graph
# 16/04/2016 geoff mclane http://geoffair.net/mperl
use strict;
use warnings;
...
# user variables
my $VERS = "0.0.6 2016-04-16";
my $load_log = 0;
my $in_file = '';
my $verbosity = 0;
my $out_file = $temp_dir."/tempkml.xg";
...
# ### DEBUG ###
my $debug_on = 0;
my $def_file = 'C:\Users\user\Downloads\fgtracker_6588208.kml';
my $def_xg = $temp_dir."/temptracker.xg";
...
\$VAR1 = {
          'TimeStamp' => {
                         'when' => '2016-04-08T01:50:36+08'
                       },
          'styleUrl' => '#Waypoints',
          'description' => '
			Coordinate: 1.760543,41.590969
			Altitude: 2363.05ft
			Speed: 129 knots',
          'Point' => {
                     'coordinates' => '1.760543,41.590969,720.26'
                   }
        };
EOF
    return $txt;
...
sub process_in_file($) {
    ...
    my $b5 = 'coordinates';
    my $b6 = 'name';
    my ($ref,$rh,$rh2,$cnt,$pt,$len,$name,$num);
    $load_log = 1;
    ##prt(Dumper($xref));
    if (! defined ${$xref}{$base}) {
        pgm_exit(1,"File $inf does not have '$base'\n");
    }
    my $aref = ${$xref}{$base};
    ##prt(Dumper($aref));
    if (! defined ${$aref}{$b2}) {
        pgm_exit(1,"File $inf does not have '$base/$b2'\n");
    }
    my $bref =  ${$aref}{$b2};
    ##prt(Dumper($bref));
    ## 'Placemark';
    if (! defined ${$bref}{$b3}) {
        pgm_exit(1,"File $inf does not have '$base/$b2/$b3'\n");
    }
    #if (! defined ${$bref}{$b6}) {
    #    pgm_exit(1,"File $inf does not have '$base/$b2/$b6'\n");
    #}
    #$name = ${$bref}{$b6};
    ...
    my $xg = '';
    my %h = ();
    foreach $ref (keys %{$cref}) {
        $name = $ref;
        # <name>#6:2016-04-08 01:26:46+08</name>
        if ($name =~ /^\#(\d+):/) {
            $num = $1;
        } else {
            prt("Found name '$name'\n");
            pgm_exit(1,"File $inf does not have '$base/$b2/$b6' of form '#6:2016-04-08 01:26:46+08'\n");
        }
        if (defined $h{$num}) {
            prtw("WARNING: Number $num is REPEATED!\n");
        }
        $rh = ${$cref}{$ref};
        if (defined ${$rh}{$b4}) {
            $rh2 = ${$rh}{$b4};
    ...
    my @arr = sort { $a <=> $b } (keys %h);
    foreach $num (@arr) {
        $pt = $h{$num};
        $xg .= "$pt\n";
    }
    $len = length($xg);
    prt("Had $cnt references...\n");
    if ($len) {
        rename_2_old_bak($out_file);
        write2file($xg,$out_file);
        prt("In $inf, XG written to $out_file\n");
    }
...
sub give_help {
	...
	prt(" Read an Google Earth kml file, source FGTracker, and generate a 2D graph of the\n");
